...
# Thêm vào 1609 edit
def display_latest_products_and_info_name(customer_name, excluded_product_ids=None):
    # Filter the DataFrame for the selected customer name, excluding the specified product IDs
    customer_data = df_review_full[df_review_full['customer_name'] == customer_name]

    # Drop duplicates based on customer and product, and sort by rating in descending order
    unique_customer_products = customer_data.drop_duplicates(subset=['customer_name', 'product_name'])
    latest_purchase = unique_customer_products.sort_values(by='customer_rating', ascending=False)
    latest_purchase = latest_purchase.drop_duplicates()  # Thêm vào 1609
...
def run_recommender_app_collab():
    ...
    if st.button("Find Recommendations"):
        if customer_input:
            if customer_input.isdigit():
                ...
                # customer_input_lower = customer_name.lower()  # Convert input to lowercase
                # customer_input_clean = unidecode(str(customer_input_lower))
                # st.subheader(f"The latest purchase of Customer  {customer_name}:")
                # excluded_product_ids_latest = []  # Initialize for display_latest_products_and_info
                # display_latest_products_and_info_name(customer_name, excluded_product_ids_latest)
                
                # st.subheader(f"Top {selected_option} Recommended Products for Customer {customer_name} (excluding latest):")
                
                # # Retrieve the latest purchase products of the customer
                # latest_purchase_products = df_collab[(df_collab['customer_name'].str.lower() == customer_input_clean) & (df_collab['customer_id'].notna())].drop_duplicates(subset='product_id')
                
                # # Filter recommendations to exclude products already in the latest purchase
                # recommendations = df_collab[df_collab['product_id'].isin(latest_purchase_products['product_id']) == False]
                # recommendations = recommendations.sample(selected_option)
                # ### end edit
                ...
                if not recommendations.empty:
                    ...
                    for i, (_, product) in enumerate(recommendations.iterrows(), start=1):
                        ...
                        with cols[i % 3]:
                            st.image(product['image'], use_column_width=True, output_format='JPEG')
                            st.write(f"Customer Name: {customer_name}")  # Display customer name
                            display_product_info(product)
    ...
    if st.button("Find Another Customer"):
        ...
        st.write("You can choose randomly customer IDs::")
        ...
        st.write("You can choose randomly customer names:")
